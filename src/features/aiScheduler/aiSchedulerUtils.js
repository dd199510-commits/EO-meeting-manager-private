...
export const DEFAULT_AI_STATE = {
  ...
  preferences: {
    avoidTimeSlots: [{ start: '12:00', end: '13:30', reason: '午餐时间' }],
    rules: [
      '周会优先安排在周一上午',
      '月会优先安排在每月第一周',
      '同一天不超过3个会议',
...
export function normalizeAiState(input) {
  ...
  return {
    ...
    preferences: {
      ...
      ...(input.preferences ?? {}),
      avoidTimeSlots: Array.isArray(input.preferences?.avoidTimeSlots)
        ? input.preferences.avoidTimeSlots
        : DEFAULT_AI_STATE.preferences.avoidTimeSlots,
      rules: Array.isArray(input.preferences?.rules)
...
export function optimizeInputForAI(inputMeetings, exportBatch = null, sourceMeetings = []) {
  ...
  const referenceMeetingsMap = buildReferenceMeetingMap(inputMeetings, exportBatch, sourceMeetings)
  const referenceIdMap = buildReferenceIdMap(referenceMeetingsMap)

  return {
    timeRange: inputMeetings.timeRange,
    meetings: inputMeetings.meetings.map((meeting) => ({
      ...
      notes: replaceMeetingReferences(
        meeting.notes ? meeting.notes.slice(0, 100) : '',
...
export function buildAIPrompt(inputMeetings, preferences, exportBatch = null, sourceMeetings = []) {
  ...
  return `你是一个专业的会议排程助手。请根据输入的会议列表和约束，输出严格可解析的 JSON。

输入会议:
${JSON.stringify(optimizedInput, null, 2)}

避免时段:
${preferences.avoidTimeSlots.map((slot) => `- ${slot.start}-${slot.end} (${slot.reason || '无'})`).join('\n')}

排程规则:
${preferences.rules.map((rule, index) => `${index + 1}. ${rule}`).join('\n')}
